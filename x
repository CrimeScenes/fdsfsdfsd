...
if getgenv().Configuration.FPSUnlock.IsEnabled then
    setfpscap(getgenv().Configuration.FPSUnlock.MaxFPS)
end

if getgenv().Configuration.ConsoleCleaner.Activate then
    while true do
        VirtualInputManager:SendKeyEvent(true, "F9", 0, game)
        wait(getgenv().Configuration.ConsoleCleaner.CleanInterval)
        VirtualInputManager:SendKeyEvent(false, "F9", 0, game)
    end
end

if getgenv().Configuration.IntroSettings.ShowIntro then
    wait(getgenv().Configuration.IntroSettings.IntroDuration)
    -- Additional code to handle what happens after the intro
end
...
local function UpdateFOV()
    ...
    -- Set the visibility of the circle based on the new configuration
    Circle.Visible = getgenv().CamLock.Radius_Visibility

    -- Set the radius of the circle from the new CamLock configuration
    Circle.Radius = getgenv().CamLock.Radius

    -- Position the circle at the mouse's position, adjusting for GUI inset
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().CamLock.activationMethod.Keybind:lower() then
        if getgenv().CamLock.isActive then
            if getgenv().CamLock.activationMethod.Type == "Toggle" then
            ...
            elseif getgenv().CamLock.activationMethod.Type == "Hold" then
                IsTargeting = true
                TargetPlayer = ClosestPlrFromMouse()
            end
        end
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if getgenv().CamLock.targetSync.UseTargeting then
            ...
            BodyPart = TargetPlayer.Character:FindFirstChild(getgenv().CamLock.targetSync.PartsToLock[1])  -- Lock onto the first part in the list
        end

        if BodyPart then
            local predictedPosition
            if getgenv().CamLock.targetSync.UseTargeting and getgenv().CamLock.AutoUnLock.Reloading then
...
-- Main Loop
RunService.RenderStepped:Connect(function()
    if getgenv().Silent.settings.Enabled and IsTargeting then  -- Only work when Silent aim is engaged
        ...
        if Target and Target.Character then
            ...
            if mostFavorablePart then
                ...
                local predictedPosition = mostFavorablePart.Position + velocity * getgenv().Silent.settings.Prediction
...
if getgenv().Adjustment.Checks.NoGroundShots then
    local function CheckNoGroundShots(Plr)
        if getgenv().Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Silent.settings.HitMode)  -- Reflects updated structure
                if TargetVelv5 then
